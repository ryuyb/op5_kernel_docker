...
static struct aa_profile *__attach_match(const char *name,
  ...
  list_for_each_entry_rcu(profile, head, base.list) {
    ...
    if (profile->xmatch && profile->xmatch_len > len) {
      unsigned int state = aa_dfa_match(profile->xmatch, DFA_START, name);
...
find_attach(struct aa_namespace *ns, struct list_head *list, const char *name) {
...
static const char *next_name(int xtype, const char *name) { return NULL; }
 ...
 */
static struct aa_profile *x_table_lookup(struct aa_profile *profile,
                                         u32 xindex) {
...
static struct aa_profile *x_to_profile(struct aa_profile *profile,
  ...
  case AA_X_NAME:
    if (xindex & AA_X_CHILD)
      /* released by caller */
      new_profile = find_attach(ns, &profile->base.profiles, name);
    else
      /* released by caller */
...
int apparmor_bprm_set_creds(struct linux_binprm *bprm) {
  ...
  unsigned int state;
  struct file_perms perms = {};
  struct path_cond cond = {file_inode(bprm->file)->i_uid,
  ...
  error = aa_path_name(&bprm->file->f_path, profile->path_flags, &buffer, &name,
                       &info);
  if (error) {
    if (unconfined(profile) || (profile->flags & PFLAG_IX_ON_NAME_ERROR))
      error = 0;
    name = bprm->filename;
    goto audit;
  }
  ...
  if (cxt->onexec) {
    ...
    cp = change_profile_perms(profile, cxt->onexec->ns, cxt->onexec->base.name,
                              AA_MAY_ONEXEC, state);
...
audit:
  error = aa_audit_file(profile, &perms, GFP_KERNEL, OP_EXEC, MAY_EXEC, name,
                        target, cond.uid, info, error);
...
void apparmor_bprm_committing_creds(struct linux_binprm *bprm) {
  ...
  if ((new_cxt->profile == profile) || (unconfined(new_cxt->profile)))
    return;
...
audit:
  if (!permtest)
    error = aa_audit_file(profile, &perms, GFP_KERNEL, OP_CHANGE_HAT,
                          AA_MAY_CHANGEHAT, NULL, target, GLOBAL_ROOT_UID, info,
                          error);
...
int aa_change_profile(const char *ns_name, const char *hname, bool onexec,
  ...
  perms =
      change_profile_perms(profile, ns, hname, request, profile->file.start);
...
audit:
  if (!permtest)
    error = aa_audit_file(profile, &perms, GFP_KERNEL, op, request, name, hname,
